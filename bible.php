...
<div id="mySidenav" class="sidenav2">
	 <a  id="about" href="index.php">Home</a>
	  <a id="blog" ><span style="font-size:20px;cursor:pointer" onclick="openNav()">Bible</span></a>
	 <a  id="projects" >
	     <form action="bible.php" method="POST" enctype="multipart/form-data">
...
<div id="mySidenav2" class="sidenav">
  ...
  <a href="#" onclick="loadbooks()">Click here</a>
...
echo '<span onclick="viewbook" class="close" title="Close Modal">&times;</span>';
 ...
 if(!empty($_POST['book']) && !empty($_POST['name']) ){
	...
	foreach($chapterz as $chapt)
	{
		...
		echo '<tr class="'.$stripe.'"><td>Chapter '.$chapt->chapter.'</td>';
			...
			echo '<td><form action="bible.php" method="POST" enctype="multipart/form-data"><input type="hidden" value="'.$chapt->chapter.'" name="chapter" />';
 ...
 if(!empty($_POST['chapter']) && !empty($_POST['name'])){
	...
	$verz = Versesred::find_chaps($book_id, $chapter);
	...
	foreach($verz as $verse)
	{
		...
		echo '<tr class="'.$stripe.'"><td><b style="color:blue">'.$chapter.'&nbsp;:&nbsp;'.$verse->verse.'</b></td>';
		echo '<td>'.$verse->text.'</td></tr>';
		$n++;
	}
	...
	echo '<form action="bible.php" method="POST" id="form1" enctype="multipart/form-data" >';
...
//	echo '</td></tr>';
 ...
 if(!empty(htmlspecialchars($_POST['id'])) && htmlspecialchars($_POST['id'])==3){
     //echo $_POST['search_field']; 
     if(preg_match("/^[\w,;'\"\s\.?!-]+$/",htmlspecialchars($_POST['search_field']))){
	$id = htmlspecialchars($_POST['id']);
	$track = empty(htmlspecialchars($_POST['search'])) ? '' : htmlspecialchars($_POST['search']);
	$code = empty(htmlspecialchars($_POST['search_field'])) ? '' : htmlspecialchars(trim($_POST['search_field']));
	$a_session = empty(htmlspecialchars($_POST['option'])) ? '' : htmlspecialchars($_POST['option']);
	
	 $searchItem = empty(htmlspecialchars($_POST['search_field'])) ? 'Please Enter the Search Word of the KJV Bible' : htmlspecialchars($_POST['search_field']);
	 echo '<tr><td colspan=3><span style="width: 80%; height: 10px;"><center>SEARCH ITEM: &nbsp;<b style="color:red">'.$searchItem.'</b></center></span></td></tr>';
	
	
	if(!empty(htmlspecialchars($_POST['search_field'])) && isset($_POST['search']) && !empty(htmlspecialchars($_POST['option'])) && htmlspecialchars($_POST['option'])==1){
		$searchText = htmlspecialchars(trim($_POST['search_field']));
	...
	if(!empty(htmlspecialchars($_POST['search_field']))  && isset($_POST['search']) && !empty(htmlspecialchars($_POST['option'])) && htmlspecialchars($_POST['option'])==2 ){
		
		$searchText = htmlspecialchars(trim($_POST['search_field']));
		//if(empty($_POST['search_field'])) $searchText = 'fasting';
		$book_id = empty($_POST['book_id']) ? 0 : $_POST['book_id'];
		
		$verzez = Verses::find_range($book_index[$lower_limit], $book_index[$page]);
		if(!empty($_POST['bookSearch']) && $_POST['bookSearch']==2) 
		
		$verzez = Verses::find_active($book_id);
	...
	if(count($bible_pages) > 1 && empty($_POST['bookSearch'])) {
		...
		elseif($current_page<22) { 
		    echo '<td><form action="bible.php" method="POST" enctype="multipart/form-data">'; 
		    echo '<input type="hidden" name="search" value="'.$track.'" />';
		    echo '<input type="hidden" name="search_field" value="'.$code.'" />';
		    echo '<input type="hidden" name="book_page" value="'.$book_index[$page].'" />';
		     echo '<input type="hidden" name="option" value="'.$a_session.'" />';
...
<div id="myNav" class="overlay" >
  ...
  <!-- <div id="myDIV">
   ...
   <form id="form" enctype="multipart/form-data" action="insertcommentary.php" method="post">
    <input type="hidden" id="logged" name="logged" value="<?php //echo $session->is_logged_in(); ?>" />
	...
	<input class="button" type="submit" value="Send">
        ...
        <center><span id="succeed" style="color: blue;"></span></center></form>
   
    </div>
   <center><button onclick="toggler()" style="display: none;" id="resp">ADD COMMENTARY</button></center> -->
...
var f = setInterval(function() {
   loadcommentary(); 
				...
				<h2>ALL REFERENCES FROM THE KING JAMES VERSION BIBLE SOCIETY OF NIGERIA (BSN)</h2> 
				<table width=100%><tr><ul>
						<td><li><!--<a href="www.indexincoporated.com">INDEX BUSINESS SOLUTIONS</a> </li>
						</td><td><li><!--<a href="">TERMS AND CONDITIONS</a> </li>
						</td><td><li><!--<a href="">PRIVACY POLICY</a></li>
						</td><td><li><!--<a href="view_result.php">EDU SERVICES</a></li></td>
						
					</ul></tr>
					<tr><td ><img src="./images/google_map.png" height="48" width="48" alt="GOOGLE"></td>
					<td ><img src="images/unical_loggo.jpg" height="48" width="48" alt="UNICAL"></td>
					<td ><!--<img src="./images/index_.png" height="48" width="48" alt="INDEX"></td>
					<td ><img src="./images/edu_logo.png" height="48" width="96" alt="LEOPORTALS NGO"></td>
					</tr>
				</table>-->
				</center>
