...
class Devotionals  {
	...
	protected static $table_name="devotionals";
  ...
  public static function find_pending($who) {
		return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE bio_id=".htmlspecialchars($who)." AND status='pending' order by datetime");
  }
  public static function find_posted($who) {
		return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE bio_id=".htmlspecialchars($who)." AND status='posted' order by datetime");
  }
   public static function find_public() {
		return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE status='posted' order by datetime, filename");
  }
  ...
  public function destroy($idz, $fil)
  {
	...
	//echo $target_path;
	if($fil=="null") return (Devotionals::delete($idz)) ? true: false;
	elseif(unlink($target_path))
	{
	    //die($target_path);
		return (Devotionals::delete($idz)) ? true: false;
	}
	else { return false;}
 ...
 public static function find_by_sql2($sql="", $two, $three) {
  ...
  public static function find_by_sql3($sql="", $bid) {
