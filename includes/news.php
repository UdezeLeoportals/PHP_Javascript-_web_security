...
class News  {
  ...
  public static function change_filename($id, $new_password){
      	self::find_by_sql( " UPDATE ".self::$table_name." SET filename = ? where id = ?", $new_password, $id);
   }
  ...
  public function destroy($idz, $fil)
  {
	...
	if($fil=="null") return (News::delete($idz)) ? true: false;
	elseif(unlink($target_path))
	{
		return (News::delete($idz)) ? true: false;
	}
	else { return false;}
