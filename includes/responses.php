...
class Responses  {
	...
	protected static $table_name="responses";
	protected static $db_fields = array('id', 'bio_id', 'topic_id', 'type', 'details', 'datetime');
  ...
  public static function count_resp($a, $post_id) {
	  global $database;
	  $sql = "SELECT COUNT(*) FROM ".self::$table_name." WHERE ( type = {$a} AND topic_id = {$post_id})";
  ...
  public static function change_filename($id, $new_password){
   ...
   public static function change_status($id, $new_password){
      	self::find_by_sql( " UPDATE ".self::$table_name." SET status = '".$new_password."' where id = ".$id);
   }
  public static function find_responds($type, $topic) {
	if($topic!="")	return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE type={$type} AND topic_id={$topic} order by datetime");
  }
  public static function find_exist($type, $topic) {
	if($topic!="")	return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE type={$type} AND details='{$topic}' order by datetime");
  }
  public static function find_posted($who) {
		return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE bio_id={$who} AND status='posted' order by datetime");
  }
  ...
  public function destroy($idz, $fil)
  {
	...
	if($fil=="") return (Scholars::delete($idz)) ? true: false;
