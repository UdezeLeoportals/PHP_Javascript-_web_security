...
class Classes {
	...
	public static function count_all($field) {
	  global $database;
	  $sql = "SELECT COUNT(*) FROM student_class WHERE unit = '{$field}' ";
 ...
 public static function find_by_dept1($dept="") {
		return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE unit = '{$dept}' ");
  }
