...
class Enebeli  {
	...
	protected static $table_name="enebeli";
	protected static $db_fields = array('id', 'fullname', 'phone', 'address', 'finger', 'table', 'details', 'delivery', 'date_time');
  ...
  public $delivery;
    ...
    public static function countcom($chapt) {
	  global $database;
	  $sql = "SELECT COUNT(*) FROM ".self::$table_name." WHERE  topic_id={$chapt}";
   ...
   public static function find_by_topic($topic="") {
		return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE topic_id={$topic} order by date_time");
  }
     public static function find_by_dept_id($dept_id="") {
		return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE dept_id={$dept_id} ");
  }
   public static function find_by_staff_no($user_id="") {
		return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE staff_no='{$user_id}' LIMIT 1");
  }
...
public static function change_message($id, $new){
   ...
   public static function get_fullname($id){
	global $database;
	$result = $database->query(" SELECT   title, last_name, middle_name, first_name FROM ".self::$table_name."   WHERE  id = '".$id."'  LIMIT 1");
	$user = $database->fetch_array($result);
	return $user['title']."  ".$user['last_name']."  ".$user['middle_name']."  ".$user['first_name'];
   }
