...
class Mailpin  {
	...
	protected static $table_name="mailpin";
	protected static $db_fields = array('id', 'email', 'pin', 'used');
   ...
   public static function find_pin($id, $pin) {
		return self::find_by_sql2("SELECT * FROM ".self::$table_name." WHERE (email = ? AND pin = ?) order by id DESC LIMIT 1", $id, $pin);
  }
  public static function check_pin($id, $pin) {
		return self::find_by_sql2("SELECT * FROM ".self::$table_name." WHERE (email = ? AND pin = ? AND used=1) order by id DESC LIMIT 1", $id, $pin);
  }
  public static function verified($id){
//	global $database;
	//$new_status = $database->escape_value($new_status);
      	self::find_by_sql( " UPDATE ".self::$table_name." SET student_id = -1 WHERE id = ".$id);
   }
  public static function delete($id){
		$result_array=self::find_by_sql("DELETE FROM ".self::$table_name." WHERE id = {$id} ");
		return !empty($result_array) ? array_shift($result_array) : false;
	}
   public static function gss_find_all() {
		return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE type=\"GSS\" ");
  }
    public static function find_gss1($unit="") {
	...
	public static function find_by_student_id($code="") {
		return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE student_id = {$code} ");
    }
 ...
 public static function find_by_sql2($sql="", $email, $pin) {
	
      global $database;
    $result_set = $database->query($sql, $email, $pin, 1);
    $object_array = array();
    while ($row = $result_set->fetch_assoc()) {
      $object_array[] = self::instantiate($row);
    }
	...
	public function create() {
	 ...
	 $sql .= ") VALUES (";
		...
		$sql .= ")";
	  if($database->query4($sql, array_values($attributes), 7)) {
