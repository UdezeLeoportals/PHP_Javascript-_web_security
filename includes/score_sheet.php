...
class Score_sheet  {
	...
	protected static $table_name="score_sheet";
	protected static $db_fields = array('id','student_id','course_id','year','first_score', 'second_score', 'third_score', 'fourth_score', 'fifth_score', 'sixth_score');
   ...
   public static function find_all() {
		return self::find_by_sql("SELECT * FROM ".self::$table_name." order by course_code, course_session desc");
  }
   
    public static function find_score($stude=0, $course=0, $col="") {
		return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE student_id = {$stude} AND $course_id={$course} AND ".$col."<40");
    }
	
	public static function find_course($student_id=0, $course_id=0) {
		return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE student_id={$student_id} AND course_id={$course_id} LIMIT 1");
    }
public static function find_repeat($student_id=0, $course_id=0, $col="") {
		return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE student_id={$student_id} AND course_id={$course_id} AND ".$col."<40");
    }
    public static function find_($student_id=0, $course_id=0) {
		return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE student_id={$student_id} AND course_id={$course_id} ");
    }
    public static function find_std_yr($stuud=0, $year=0) {
		return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE student_id={$stuud} AND year={$year} ");
    }
    public static function find_if_reg($matric="", $session="", $code="") {
		return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE matric_no=\"{$matric}\" AND session=\"{$session}\" AND code=\"{$code}\" LIMIT 1");
    }
   ...
   public static function change_status($id, $new_status){
    ...
    public static function fil_score($id, $new_score=0, $col){
	global $database;
	$new_status = $database->escape_value($new_score);
      	self::find_by_sql( " UPDATE ".self::$table_name." SET ".$col." = ".$new_score." where id = ".$id);
   }
