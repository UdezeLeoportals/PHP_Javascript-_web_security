...
class Jobs  {
	...
	protected static $db_fields = array('id', 'bio_id', 'topic', 'details', 'filename',  'linker', 'datetime', 'status', 'employer', 'location', 'deadline');
  ...
  public static function change_filename($id, $new_password){
   ...
   public static function change_status($id, $new_password){
      return	self::find_by_sql3( " UPDATE ".self::$table_name." SET status = ? where id =? ", $new_password, $id);
   }
  public static function find_pending($who) {
		return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE bio_id=? AND status='pending' order by datetime", $who);
  }
  public static function find_posted($who) {
		return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE bio_id=? AND status='posted' order by deadline", $who);
  }
   public static function find_public() {
		return self::find_by_sql2("SELECT * FROM ".self::$table_name." WHERE status='posted' order by deadline");
  }
   public static function find_by_id($id=0) {
    $result_array = self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE id=?  LIMIT 1", $id);
  ...
  public function destroy($idz, $fil)
  {
	...
	if($fil=="null") return (Jobs::delete($idz)) ? true: false;
	elseif(unlink($target_path))
	{
		return (Jobs::delete($idz)) ? true: false;
	}
	else { return false;}
  ...
  public static function find_by_student_id($id=0) {
   ...
   return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE chat_id={$id} LIMIT 1 ");
   // return !empty($result_array) ? array_shift($result_array) : false;
  }
...
public static function find_by_sql2($sql="") {
  ...
  public static function find_by_sql3($sql="", $two, $three) {
  ...
  public static function find_by_sql4($sql="", $one) {
	...
	$sql .= ") VALUES (";
		...
		$sql .= ")";
	  if($database->query4($sql, array_values($attributes), 2)) {
