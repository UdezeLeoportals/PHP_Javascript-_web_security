...
class Course_allocation  {
	...
	protected static $table_name="course_allocation";
	protected static $db_fields = array('id', 'staff_id', 'course_id', 'semester_id', 'status');
  ...
  public static function find_staff($staff_id=0,$semester_id=0) {
		return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE staff_id={$staff_id} AND semester_id={$semester_id} order by course_id");
  }
  public static function find_by_staff($staff_id=0) {
		return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE staff_id={$staff_id} order by semester_id");
  } 
