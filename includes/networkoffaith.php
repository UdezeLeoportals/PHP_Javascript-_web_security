...
class Networkoffaith  {

	protected static $table_name="networkoffaith";
	protected static $db_fields = array('id', 'field', 'content');
   ...
   public static function find_all() {
    ...
    public static function find_field($unit="") {
		return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE field='{$unit}' LIMIT 1");
    }
    public static function find_conclude($per_page, $offset) {
		return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE id>10 LIMIT ".$per_page." OFFSET ".$offset);
    }
	public static function find_HOD($dept_id, $type='dept', $sems=0, $office='head') {
		return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE unit_id={$dept_id} AND unit_type='{$type}' AND semester_id={$sems} AND Office='{$office}' LIMIT 1");
    }
