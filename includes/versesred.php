...
class Versesred  {
	...
	protected static $table_name="versesred";
  ...
  public static function find_range($bid, $bup) {
		return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE (book_id>{$bid} AND book_id<= {$bup}) order by id ASC");
  }
  public static function find_active($per_page) {
		return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE book_id = {$per_page} order by id ASC ");
    }
  ...
  public static function find_a_verse($book=0,$chapt=0,$vers=0) {
		return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE book_id={$book} AND chapter={$chapt} AND verse={$vers} LIMIT 1");
  }
