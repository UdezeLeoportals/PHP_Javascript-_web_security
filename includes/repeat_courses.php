...
class Repeat_courses {

	protected static $table_name="repeat_courses";
	protected static $db_fields = array('id', 'matric_no', 'course_code', 'credit_unit', 'no_of_times', 'session', 'semester');
   ...
   public static function find_student($matric="", $course_session="", $semester=""){
		return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE ( matric_no = \"{$matric}\" AND session = \"{$course_session}\" AND semester = \"{$semester}\" ) ");
   }
public static function check_student($matric="", $course_session=""){
		return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE matric_no = \"{$matric}\" AND session = \"{$course_session}\"  ");
   }
   public static function find_carry($matric=""){
		return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE matric_no = \"{$matric}\" AND no_of_times=3 ");
   }
   public static function check_course($matric="", $course_session="", $code=""){
		return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE matric_no = \"{$matric}\" AND session = \"{$course_session}\" AND course_code='{$code}' LIMIT 1");
   }
public static function find_my_repeat($matric="", $course_session="", $code=""){
		return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE matric_no = \"{$matric}\" AND session = \"{$course_session}\" AND course_code=\"{$code}\" ");
   }
   public static function find_by_session($matric="", $course_session=""){
		return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE matric_no = \"{$matric}\" AND session = \"{$course_session}\" ");
   }
   public static function update_times($matric="", $course_session="", $code="", $new){
		return self::find_by_sql("UPDATE ".self::$table_name." SET no_of_times= {$new} WHERE matric_no = \"{$matric}\" AND session = \"{$course_session}\" AND course_code=\"{$code}\" ");
   }
   
  public static function find_by_code($code="", $session="") {
		return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE course_code=\"{$code}\" AND session=\"{$session}\" LIMIT 1");
  }
