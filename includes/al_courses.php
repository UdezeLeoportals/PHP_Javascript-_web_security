...
class Al_courses  {
	...
	protected static $table_name="al_courses";
	protected static $db_fields = array('id', 'staff_no', 'course_code', 'course_title', 'a_session','semester', 'status', 'HOD_approval');
  ...
  public static function find_staff($staff_no="", $session="", $semester="") {
		return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE staff_no=\"{$staff_no}\" AND a_session=\"{$session}\" AND semester=\"{$semester}\" order by course_code");
  } 
  ...
  public static function find_by_code( $session="", $code="") {
		return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE a_session=\"{$session}\" AND course_code=\"{$code}\" AND status=\"CO-ORDINATOR\" LIMIT 1");
  }
   public static function find_coordinator($course="", $session="", $semester="") {
		return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE course_code=\"{$course}\" AND a_session=\"{$session}\" AND semester=\"{$semester}\"  AND status=\"CO-ORDINATOR\"");
  }
