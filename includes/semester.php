...
class Semester {
	...
	protected static $db_fields = array('id', 'a_session', 'name', 'status', 'closed');
  ...
  public static function find_active() {
    ...
    public static function update_stat($state, $id){
      	return self::find_by_sql( " UPDATE ".self::$table_name." SET closed = ".$state." where id = ".$id);
    }
      public static function find($session, $semester) {
		return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE name = '{$semester}' AND a_session = '{$session}' LIMIT 1 ");
    }
